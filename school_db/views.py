...
def problem_two(request):
    ...
    instructors = Instructor.objects.filter(
        hire_date__year__lt=2010).order_by('hire_date')
    for instructor in instructors:
        print('\nFull Name:', instructor.first_name, instructor.last_name)
        print('Hire Date:', instructor.hire_date)
...
def problem_four(request):
    ...
    print('Instructors count:', instructors)
...
def problem_five(request):
    ...
    new_student = Student.objects.create(
        first_name='Homer', last_name='Simpson', year=10, gpa=1.5)
    print('Id:', new_student.pk)
    print('Full name:', new_student.first_name, new_student.last_name)
    print('Year', new_student.year)
...
def problem_six(request):
    ...
    student_id = 11
    Student.objects.filter(pk=student_id).update(gpa=2.5)
...
FROM `school_db_instructor` LEFT OUTER JOIN `school_db_course`
